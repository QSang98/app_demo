package com.example.demoapp

object Constant {
    const val EMAIL = "t6@grr.la"
    const val PWD = "3d4f2bf07dc1be38b20cd6e46949a1071f9d0e3d"
    const val APPLICATION = "lismi"
    const val APPLICATION_TYPE = 3
    const val APPLICATION_VERSION = "3.9"
    const val DEVICE_ID = "14e26db6ecded593"
    const val DEVICE_NAME = "samsung SM-J600G"
    const val DEVICE_TYPE = 1
    const val OS_VERSION = "10"
    const val API = "login_version_2"
    const val API_USER = "get_user_inf"
}